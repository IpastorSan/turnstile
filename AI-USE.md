...
| `docs/brand/cover-background-fal.png`, `docs/brand/cover-1920x1080.png` | The submission cover. **The background art is AI-generated** with FAL (`fal-ai/flux-pro/v1.1-ultra`), three seeds tried, one kept. Everything legible on the cover is not: the title, tagline and URL are set in the site's own fonts by `docs/brand/render-cover.mjs` in headless Chromium, over that background. Added 2026-09-13. |
| `docs/brand/logo.svg`, `web/app/icon.svg`, `web/app/favicon.ico` | The logo and favicon. Not generated: they are the fare-gate mark already drawn in the site header (`web/components/Rail.tsx`), redrawn as SVG, with the favicon cut on a 16px grid so it stays sharp. See `docs/brand/README.md`. Added 2026-09-13. |
| `docs/demo/turnstile-demo.mp4` | The submission video. **The narration is Ignacio's own voice**, nine takes, no AI voice or text to speech. The picture was produced by scripts an agent wrote: Playwright recordings of the live site and explorers, real terminal sessions replayed byte for byte, and three HTML slides (the title slide reuses the FAL-generated cover background). An ffmpeg script fitted picture to voice by trimming or holding frames, never by changing speed. Scripts and details in `docs/demo/`. Added 2026-09-13. |
