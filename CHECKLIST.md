...
- [x] Working app, tested via the **Sandbox App** — everything but the proof is live and verified through Caddy: the context route signs with the real key, the widget mounts, both failure paths return honest refusals. **The proof itself needs the phone**, which is the one step that cannot be automated
...
- **Update (2026-09-11, MOV-277):** the first real Sandbox App proof landed at **2026-09-11 09:06:02 UTC** (`verified_at` 1789117562 in the live store), verified by World's Developer Portal, so the Sandbox App row above is ticked. It was **orphaned by a key mismatch**: `/onboard` posted `liquidity.turnstile.eth` and the verify route stored the proof under that name, while the market joins `world_verification` on the ERC-8004 agent uid (`eip155:11155111:0x8004a818…/10127`). So `/api/sellers` kept reporting our seller as `unknown`. Verified 2026-09-11 against the live store: exactly one row, keyed by the ENS name. Fixed in MOV-277: the route resolves the listing to its agent uid server-side (`identity/canonical.ts`), `/onboard` picks listings from the store and allows unregistered subnames as labelled reservations, and `scripts/world-rekey.ts` moves the existing row. **Until that script runs on the host, the live market still shows `unknown`**; the next verification of the same listing also moves it.
  - **Update (2026-09-13, MOV-288):** the script ran on the host on 2026-09-11 (backup `graph/sink/data/discovery.db.bak-mov277`). `/api/sellers` reports `liquidity.turnstile.eth` as `worldVerification: verified`, checked again from outside on 2026-09-13. The "until that script runs" sentence above is resolved.
...
- [x] **Upstream contribution, merged (added 2026-09-09, MOV-271).** Not a listed gate, but it is the strongest thing this submission has and a judge will not find it unless we say so: [`uniswap-ai#148`](https://github.com/Uniswap/uniswap-ai/issues/148), which we filed, was fixed by Uniswap in [`#149`](https://github.com/Uniswap/uniswap-ai/pull/149) and **merged the same day** — 7 files across the skill, its docs page, the plugin `CLAUDE.md` and both eval rubrics. **Say in the submission that the merged code is theirs and the report was ours.** Overclaiming a merged PR when the author is `wkoutre` is checkable in one click and would cost more than the claim is worth
...
- [x] Recipe using **both**, final result depends meaningfully on both
  ...
  - Ticked 2026-09-11 (MOV-281): published on the user's go-ahead, and listed in the public
    catalog at `https://api.bazantic.com/v1/recipes/uniswap-pool-safety-analysis` (author
    Ignacio Pastor), read back the same evening.
