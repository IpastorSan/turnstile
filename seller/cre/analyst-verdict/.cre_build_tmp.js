...
function calibrationFrom(overrides) {
  return { ...THRESHOLDS, ...overrides ?? {} };
}
var SECONDS_PER_YEAR = 365 * 24 * 3600;
function usd(value2) {
  if (value2 === null || !Number.isFinite(value2))
    return "unknown";
  const abs = Math.abs(value2);
  if (abs >= 1e9)
    return `$${(value2 / 1e9).toFixed(2)}B`;
  if (abs >= 1e6)
    return `$${(value2 / 1e6).toFixed(2)}M`;
  if (abs >= 1000)
    return `$${(value2 / 1000).toFixed(1)}k`;
  if (abs >= 1)
    return `$${value2.toFixed(2)}`;
  return `$${value2.toFixed(6)}`;
}
function pct(value2, digits = 2) {
  if (value2 === null || !Number.isFinite(value2))
    return "unknown";
  return `${(value2 * 100).toFixed(digits)}%`;
}
...
function slippageOf(rung, referenceRate) {
  if (rung.executedRate === null || referenceRate === null || referenceRate === 0)
    return null;
  return 1 - rung.executedRate / referenceRate;
}
function referenceRate(depth) {
  for (const rung of depth.rungs) {
    if (rung.executedRate !== null && rung.executedRate > 0)
      return rung.executedRate;
  }
  return null;
}
function usableDepthUSD(depth, cal) {
  ...
  for (const rung of depth.rungs) {
    ...
    if (slip === null)
      continue;
    if (slip <= cal.usableDepthSlippageCeiling)
      best = rung.notionalUSD;
  }
  return best;
}
function rungAtOrAbove(depth, notionalUSD) {
  for (const rung of depth.rungs)
    if (rung.notionalUSD >= notionalUSD)
      return rung;
  return null;
}
function ticksPhrase(ticks) {
  if (ticks === null)
    return "";
  return `, ${ticks} initialized tick${ticks === 1 ? "" : "s"} crossed`;
}
function historyWindow(pool, hours) {
...
function inventoryBalance(pool, cal) {
  ...
  for (const token of pool.tokens) {
    ...
    evidence[`${token.symbol} side (${short})`] = `${token.balance.toLocaleString("en-US", { maximumFractionDigits: 4 })} ${token.symbol}` + `, ${token.decimals} decimals` + ` (${usd(token.balanceUSD)}${token.priceUSD === null ? ", unpriced" : ""})`;
  }
  if (priced.length < 2) {
    return {
      ...
      evidence,
      reasoning: "Balance is a USD comparison between the sides. With a side unpriced there is no " + "comparison to make, and guessing one would be the same as inventing the answer."
    };
  }
  const total = priced.reduce((sum, t) => sum + (t.balanceUSD ?? 0), 0);
  const thin = priced.reduce((min, t) => (t.balanceUSD ?? 0) < (min.balanceUSD ?? 0) ? t : min, priced[0]);
  ...
  if (share < cal.oneSidedFailShare) {
    return {
      ...
      verdict: "fail",
      structural: true,
      headline: `The pool is effectively one-sided: ${thin.symbol} is ${pct(share, 6)} of the reserves ` + `(${usd(thin.balanceUSD)} against ${usd(total)} claimed).`,
      evidence,
      reasoning: `Providing liquidity here means depositing against a reserve that is almost entirely ` + `${pool.tokens.find((t) => t.symbol !== thin.symbol)?.symbol ?? "one token"}. Every trade ` + `that arrives buys the scarce side out and pays you in the abundant one, so the position ` + `converges to holding the token nobody wants. The headline TVL is arithmetically correct ` + `and economically meaningless — it is one token's balance multiplied by a price that this ` + `same imbalanced pool is what sets.`
    };
  }
  if (share < cal.oneSidedWarnShare) {
    return {
      ...
      evidence,
      reasoning: "A concentrated-liquidity pool drifts one-sided as price leaves the active range, so this " + "is not automatically wrong — but it does mean an LP entering now is taking a directional " + "position rather than a neutral fee-earning one."
    };
  }
  return {
    id: "inventory-balance",
    ...
    evidence,
    reasoning: "A genuinely two-sided reserve means arriving flow trades in both directions, which is the " + "precondition for earning fees rather than accumulating one token."
  };
}
function executableDepth(depth, cal) {
  const base = { id: "executable-depth", label: "Executable depth", structural: true };
  if (depth === null) {
    return {
      ...base,
      verdict: "unknown",
      ...
      evidence: {},
      reasoning: "Every other signal in this report is derived from indexed history. Depth is the only one " + "that asks the pool a question at the current block, and without it a verdict rests " + "entirely on what the pool used to be."
    };
  }
  const reference = referenceRate(depth);
  const evidence = {
    ...
    "notional priced with": `${depth.notionalPricing.tokenSymbol} at ` + `${usd(depth.notionalPricing.priceUSD)} (${depth.notionalPricing.priceSource})`
  };
  for (const rung of depth.rungs) {
    const slip = slippageOf(rung, reference);
    evidence[usd(rung.notionalUSD)] = rung.amountOut === null ? `did not execute — ${rung.error ?? "no reason given"}` : `${rung.amountOut.toLocaleString("en-US", { maximumFractionDigits: 6 })} ` + `${depth.tokenOutSymbol}, slippage ${pct(slip, 3)}` + ticksPhrase(rung.ticksCrossed);
  }
  ...
  if (reference === null || smallest !== null && smallest.amountOut === null) {
    return {
      ...base,
      verdict: "fail",
      headline: `The pool cannot fill a ${usd(cal.minimumViableNotionalUSD)} trade — the quote ` + `reverted at the smallest size asked for.`,
      evidence,
      reasoning: "A pool that reverts on a retail-sized swap has no liquidity in its active range. Whatever " + "the indexer reports as locked value is not reachable by a trade, and fees are only " + "earned by trades."
    };
  }
  const retail = rungAtOrAbove(depth, 1e4);
  ...
  if (retailSlip !== null && retailSlip > cal.retailSlippageFail) {
    return {
      ...
      evidence,
      reasoning: "At that cost, informed flow routes elsewhere and what is left is mostly adverse — the " + "trades that do arrive are the ones that only make sense against a mispriced pool. That " + "is the flow an LP loses to."
    };
  }
  if (retailSlip !== null && retailSlip > cal.retailSlippageWarn) {
    return {
      ...
      evidence,
      reasoning: "The pool executes, but not competitively. Routers will prefer a deeper venue for anything " + "above small size, which caps the fee flow an LP can expect regardless of how much capital " + "is sitting in the pool."
    };
  }
  return {
    ...base,
    verdict: "pass",
    headline: `Real trades execute: ${usable === null ? "small size fills" : usd(usable) + " fills within 1% slippage"}` + (retailSlip === null ? "" : `, and ${usd(retail.notionalUSD)} costs ${pct(retailSlip, 3)}`) + ".",
    evidence,
    reasoning: "This is the only measurement here taken against the live pool rather than against indexed " + "history, and it is the one an LP is actually exposed to: it is the flow this pool can win."
  };
}
function depthVersusTvl(pool, depth, cal) {
  const base = { id: "depth-vs-tvl", label: "Depth against claimed TVL", structural: true };
  ...
  if (depth === null) {
    return {
      ...
      reasoning: "TVL is what an indexer computed from reserves and a derived price. Whether any of it is " + "reachable by a trade is a separate question, and only a quote answers it."
    };
  }
  const share = usable !== null && pool.totalValueLockedUSD > 0 ? usable / pool.totalValueLockedUSD : null;
  const evidence = {
    "claimed TVL": usd(pool.totalValueLockedUSD),
  ...
  if (usable === null || share !== null && share < cal.depthToTvlFailShare) {
    return {
      ...base,
      verdict: "fail",
      headline: `Claimed TVL of ${usd(pool.totalValueLockedUSD)} does not survive contact with a quote: ` + `${usable === null ? "nothing" : usd(usable)} is reachable within 1% slippage.`,
      evidence,
      reasoning: "The gap is the finding. A ranking by TVL puts this pool near the top and a ranking by " + "what a trade can actually do puts it nowhere, which is exactly the trap this analyst " + "exists to catch. TVL is derived from a price; depth is derived from the pool."
    };
  }
  if (share !== null && share < cal.depthToTvlWarnShare) {
    return {
      ...
      evidence,
      reasoning: "In concentrated liquidity most capital sits outside the active range by design, so a small " + "ratio is normal — but it does mean the headline number overstates the fee-earning capital " + "by two orders of magnitude, and an LP sizing a position off TVL will be disappointed."
    };
  }
  return {
    ...base,
    ...
    evidence,
    reasoning: "The indexed number and the live pool are describing the same object, which is the baseline " + "assumption every other historical signal here depends on."
  };
}
function slippageCurve(depth, cal) {
  const base = { id: "slippage-curve", label: "Slippage curve", structural: false };
  if (depth === null || depth.rungs.length < 3) {
    return {
      ...
      evidence: {},
      reasoning: "The shape of the curve — not any single point on it — is what tells you whether the " + "active range is deep or merely present."
    };
  }
  const reference = referenceRate(depth);
  const points = [];
  for (const rung of depth.rungs) {
    const slip = slippageOf(rung, reference);
    if (slip !== null)
      points.push({ notionalUSD: rung.notionalUSD, slip, ticks: rung.ticksCrossed });
  }
  const evidence = {};
  for (const p of points) {
    evidence[usd(p.notionalUSD)] = `${pct(p.slip, 3)}${p.ticks === null ? "" : ` (${p.ticks} tick${p.ticks === 1 ? "" : "s"})`}`;
  }
  if (points.length < 2) {
    return {
      ...base,
      verdict: "unknown",
      headline: `Only ${points.length} of ${depth.rungs.length} quoted sizes filled, so there is no ` + `curve to read.`,
      evidence: {
        ...evidence,
        "rungs that did not fill": depth.rungs.filter((r) => r.amountOut === null).map((r) => usd(r.notionalUSD)).join(", ") || "none"
      },
      reasoning: "The shape of the curve is what says where liquidity runs out, and a shape needs at " + "least two points. Whether the pool trades at all is answered above, not here."
    };
  }
  let cliff = null;
  for (let i2 = 1;i2 < points.length; i2++) {
    const previous = points[i2 - 1];
    const current = points[i2];
    if (previous.slip <= cal.usableDepthSlippageCeiling && current.slip >= cal.retailSlippageFail * 5) {
      cliff = {
  ...
  if (cliff) {
    return {
      ...base,
      verdict: "warn",
      headline: `Liquidity runs out between ${usd(cliff.from)} and ${usd(cliff.to)}: ${cliff.describe} ` + `across that one step.`,
      evidence,
      reasoning: "A cliff is the edge of the active range. Below it the pool is competitive; above it a " + "trade walks through ticks with nothing in them. For an LP that means fee income is capped " + "at the flow that fits under the cliff, and it marks how far price can move before the " + "position is entirely on one side."
    };
  }
  ...
  return {
    ...base,
    verdict: "pass",
    headline: `Slippage scales smoothly with size, reaching ${pct(last.slip, 3)} at ${usd(last.notionalUSD)}.`,
    evidence,
    reasoning: "No cliff inside the quoted range means the active liquidity is continuous across every size " + "tested, so the pool degrades gracefully rather than emptying at one particular size."
  };
}
function feeReturn(pool, cal) {
  const base = { id: "fee-return", label: "Fee return to LPs", structural: false };
  ...
  if (pool.hourly.length === 0 || pool.totalValueLockedUSD <= 0) {
    return {
      ...
      reasoning: "The Messari schema writes interval snapshots only at rollover, so a pool indexed very " + "recently has a currently-open interval and nothing closed. Annualizing from the " + "cumulative figure instead would silently divide by an indexing window rather than by " + "elapsed time, which is how a two-day-old subgraph produces a fictional APY."
    };
  }
  ...
  const averageTvl = window.buckets.length > 0 ? window.buckets.reduce((sum, h) => sum + h.totalValueLockedUSD, 0) / window.buckets.length : pool.totalValueLockedUSD;
  ...
  if (apy > cal.feeApyWarnHigh) {
    return {
      ...
      evidence,
      reasoning: "Yields at this level are normally one large trade extrapolated across a year, or wash " + "volume. Either way the number will not persist, and sizing a position against it is the " + "mistake the number is designed to produce."
    };
  }
  if (apy < cal.feeApyWarnLow) {
    return {
      ...
      evidence,
      reasoning: "Fee income is the entire compensation for impermanent loss. Below roughly 1% annualized " + "the position is a directional bet on the pair with a rounding error attached."
    };
  }
  return {
    ...base,
    ...
    evidence,
    reasoning: "This is computed from supply-side revenue the subgraph actually recorded in closed " + "intervals, not from an advertised fee tier multiplied by hoped-for volume, so it is what " + "LPs were paid rather than what they might be."
  };
}
function activityContinuity(pool, cal) {
  const base = { id: "activity-continuity", label: "Trading continuity", structural: true };
  const window = historyWindow(pool, cal.activityWindowHours);
  if (pool.hourly.length === 0) {
    return {
      ...
      reasoning: "Either the pool is new or the subgraph has not indexed far enough past its start block " + "to close an interval. Both are reasons to withhold a historical judgement rather than to " + "make one from nothing."
    };
  }
  if (window.buckets.length === 0) {
    return {
      ...base,
      verdict: "fail",
      headline: `Nothing traded in the ${cal.activityWindowHours} hours before the subgraph head, ` + `though the pool does have older history.`,
      evidence: {
      ...
      reasoning: "A pool that has stopped trading still holds an LP's capital and still reprices against " + "every move in the pair. All of the risk, none of the fees."
    };
  }
  ...
  if (share <= cal.activeHourFailShare) {
    return {
      ...base,
      verdict: "fail",
      headline: `Trading is not continuous: ${window.activeHours} of ${window.elapsedHours} elapsed hours ` + `saw any volume.`,
      evidence,
      reasoning: "An LP is exposed to the pool every hour and paid only in the hours it trades. A pool that " + "is idle most of the time carries the full inventory risk of a live market and earns a " + "fraction of a live market's fees."
    };
  }
  if (share < cal.activeHourWarnShare) {
    return {
      ...base,
      verdict: "warn",
      headline: `Volume arrives in bursts: ${window.activeHours} of ${window.elapsedHours} elapsed hours traded.`,
      evidence,
      reasoning: "Intermittent flow makes realized fee income lumpy and much harder to forecast than the " + "annualized figure suggests, and it usually means a single router or bot is the pool."
    };
  }
  return {
    ...base,
    ...
    evidence,
    reasoning: "Continuous two-way flow is what turns a fee tier into income. It also means the price is " + "being kept honest by arbitrage rather than drifting between rare trades."
  };
}
function lpConcentration(pool, cal) {
  const base = { id: "lp-concentration", label: "LP concentration", structural: false };
  const evidence = {
  ...
  if (pool.positionCount === 0) {
    return {
      ...
      evidence,
      reasoning: "Positions reach this subgraph through the NonfungiblePositionManager, so liquidity " + "provided directly to the pool contract — or before the indexer start block — is invisible " + "here. Absence is not evidence of absence."
    };
  }
  if (pool.openPositionCount <= cal.lpCountFail) {
    return {
      ...
      evidence,
      reasoning: "With a single LP there is no market to join — you would be trading against whoever set the " + "pool up, on terms they chose, and they can withdraw the whole range in one transaction."
    };
  }
  if (pool.openPositionCount <= cal.lpCountWarn) {
    return {
      ...
      evidence,
      reasoning: "A small LP set means one withdrawal can move the depth profile materially, so the depth " + "measured above is less durable than it looks."
    };
  }
  return {
    ...base,
    ...
    evidence,
    reasoning: "A broad LP base means the depth profile is the aggregate of many independent decisions and " + "does not disappear when one of them changes their mind."
  };
}
function confidenceOf(input, signals, lagSeconds, cal) {
  let confidence = 1;
...
function ratingOf(signals) {
  ...
  if (structuralUnknowns >= 2)
    return "INSUFFICIENT_DATA";
  if (fails > 0 || warns >= 2)
    return "CAUTION";
  return "ACCEPTABLE";
}
function summarize(pool, rating, confidence, signals, lagSeconds, hasDepth, cal) {
  const fails = signals.filter((s) => s.verdict === "fail");
  const warns = signals.filter((s) => s.verdict === "warn");
  const parts = [];
  if (rating === "AVOID") {
    parts.push(`Do not provide liquidity to ${pool.name}. ` + fails.map((s) => s.headline).join(" ") + " Any one of those is disqualifying on its own; the headline TVL does not offset them, " + "because it is derived from the same reserves that produced them.");
  } else if (rating === "INSUFFICIENT_DATA") {
    parts.push(`No verdict on ${pool.name}. The measurements that decide whether a pool is a venue at all ` + `could not be taken, and a confident-sounding answer built on the ones that did land would ` + `be worse than none.`);
  } else if (rating === "CAUTION") {
    parts.push(`${pool.name} works as a venue but is not a clean LP position. ` + [...fails, ...warns].map((s) => s.headline).join(" "));
  } else {
    parts.push(`${pool.name} looks safe to LP on the evidence available. Reserves are genuinely two-sided, ` + `live quotes execute at the sizes tested, and the fee return is realized rather than ` + `projected.`);
  }
  if (!hasDepth) {
    parts.push("This rests on indexed history alone — no live quote was obtained, so nothing here has been " + "checked against the pool as it stands right now.");
  }
  if (lagSeconds > cal.lagStaleSeconds) {
    parts.push(`The subgraph is ${duration(lagSeconds)} behind the chain head, so the historical half of ` + `this verdict describes the pool as it was, not as it is. Where the live quote and the ` + `history disagree, believe the quote.`);
  } else if (lagSeconds > cal.lagWarnSeconds) {
...
function assess(input, calibration) {
  ...
  for (const signal of signals) {
    if (signal.verdict === "unknown")
      caveats.push(`${signal.label}: ${signal.headline}`);
  }
  if (lagSeconds > cal.lagWarnSeconds) {
    caveats.push(`Subgraph head is block ${pool.source.blockNumber}, ${duration(lagSeconds)} behind the ` + `chain head. Every historical figure above is as of that block.`);
  }
  if (depth && depth.notionalPricing.priceSource === "subgraph-lastPriceUSD") {
    caveats.push(`USD notionals on the depth ladder were sized using the subgraph's own derived price for ` + `${depth.notionalPricing.tokenSymbol} (${usd(depth.notionalPricing.priceUSD)}). For a pool ` + `whose price the subgraph derives from this same pool, that is circular — the token ` + `amounts quoted are exact, the dollar labels on them are only as good as that price.`);
  }
  if (pool.tokens.some((t) => t.priceUSD === null)) {
    caveats.push(`The subgraph could not anchor a USD price for ` + pool.tokens.filter((t) => t.priceUSD === null).map((t) => t.symbol).join(", ") + `, so any USD figure involving that side is incomplete rather than zero.`);
  }
  return {
    pool: pool.name,
...
var VERDICT_ABI_PARAMS = "address pool, uint8 rating, uint16 confidenceBp, uint8 failMask, uint8 warnMask, uint64 assessedAt, bytes32 evidenceHash";
function maskOf(signals, want) {
  let mask = 0;
  for (const signal of signals) {
    const bit = SIGNAL_BITS.indexOf(signal.id);
    if (bit === -1)
      throw new Error(`signal '${signal.id}' has no bit in SIGNAL_BITS`);
    if (signal.verdict === want)
      mask |= 1 << bit;
  }
  return mask;
}
function compact(verdict, evidenceHash, assessedAt) {
  const confidenceBp = Math.max(0, Math.min(1e4, Math.round(verdict.confidence * 1e4)));
  return {
    poolAddress: verdict.poolAddress.toLowerCase(),
...
function encodeVerdictReport(v) {
  return encodeAbiParameters2(parseAbiParameters2(VERDICT_ABI_PARAMS), [
    v.poolAddress,
    v.ratingCode,
    v.confidenceBp,
    v.failMask,
...
function signalsInMask(mask) {
  return SIGNAL_BITS.filter((_, bit) => (mask & 1 << bit) !== 0);
}
function describeCompact(v) {
  ...
  if (fails.length > 0)
    parts.push(`failing ${fails.join(", ")}`);
  if (warns.length > 0)
    parts.push(`warning on ${warns.join(", ")}`);
  return parts.join("; ");
}
var configSchema = exports_external.object({
  schedule: exports_external.string(),
  evidenceUrl: exports_external.string(),
  evidenceSecretId: exports_external.string(),
  calibrationSecretId: exports_external.string(),
  compareToPublicCalibration: exports_external.boolean(),
  evm: exports_external.object({
    chainSelectorName: exports_external.string(),
    verdictConsumer: exports_external.string(),
    gasLimit: exports_external.string().optional()
  })
});
var ZERO_ADDRESS = "0x0000000000000000000000000000000000000000";
function fetchEvidence(runtime2, token) {
  const response = new cre.capabilities.HTTPClient().sendRequest(runtime2, {
    url: runtime2.config.evidenceUrl,
    method: "GET",
    multiHeaders: {
...
var onCronTrigger = (runtime2) => {
  ...
  const evidenceToken = runtime2.getSecret({ id: config.evidenceSecretId }).result().value;
  const calibrationRaw = runtime2.getSecret({ id: config.calibrationSecretId }).result().value;
  const body = fetchEvidence(runtime2, evidenceToken);
  ...
  runtime2.log(`evidence: ${body.length} bytes for ${input.pool.name}, keccak256 ${evidenceHash}`);
  runtime2.log(calibration === null ? "calibration: NONE — no Vault override, scoring with the public thresholds" : `calibration: ${Object.keys(calibration).length} sealed threshold overrides applied`);
  runtime2.log(`verdict: ${describeCompact(settled)}`);
  if (config.compareToPublicCalibration && calibration !== null) {
    const publicVerdict = assess(input);
    const agrees = publicVerdict.rating === verdict.rating && Math.round(publicVerdict.confidence * 100) === Math.round(verdict.confidence * 100);
    runtime2.log(agrees ? `public calibration agrees on ${publicVerdict.rating} — the edge did not change this call` : `public calibration says ${publicVerdict.rating} at ${Math.round(publicVerdict.confidence * 100)}%, ` + `the sealed one says ${verdict.rating} at ${Math.round(verdict.confidence * 100)}% — ` + "this is the difference the Vault secret bought");
  }
  const donRuntime = runtime2.usingTheDons();
  return settleOnChain(donRuntime, config, settled);
};
function settleOnChain(runtime2, config, settled) {
  const summary = describeCompact(settled);
  if (config.evm.verdictConsumer.toLowerCase() === ZERO_ADDRESS) {
    runtime2.log("verdictConsumer is the zero address — skipping the on-chain write");
  ...
  const evmClient = new cre.capabilities.EVMClient(network295.chainSelector.selector);
  ...
  const write2 = evmClient.writeReport(runtime2, {
    receiver: config.evm.verdictConsumer,
    report: report2,
    gasConfig: config.evm.gasLimit ? { gasLimit: config.evm.gasLimit } : undefined
  }).result();
  if (write2.txStatus !== TxStatus.SUCCESS) {
    throw new Error(`verdict write failed: ${write2.errorMessage || write2.txStatus}`);
  }
  if (write2.receiverContractExecutionStatus !== undefined && write2.receiverContractExecutionStatus !== 0) {
    throw new Error(`VerdictConsumer reverted: status ${write2.receiverContractExecutionStatus}`);
  }
  const txHash = bytesToHex3(write2.txHash || new Uint8Array(32));
  runtime2.log(`settled on ${config.evm.chainSelectorName}: ${txHash}`);
